  ...
  return (
    <div className="flex flex-col lg:flex-row items-center gap-12 py-12">
      ...
      <div className="flex-1 space-y-6 w-full">
        <h3 className="text-3xl serif font-bold text-slate-900 mb-6">{t('arch.title')}</h3>
              ...
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-3">
                  <span className={`px-3 py-1 rounded text-xs font-bold uppercase tracking-wider ${layer.color}`}>
                    {t(`arch.layers.${layer.id}.name`)}
                  </span>
                  <span className="font-serif font-bold text-slate-800">{t(`arch.layers.${layer.id}.analogy`)}</span>
                </div>
                {activeLayer === layer.id && <ArrowDownUp size={16} className="text-slate-400" />}
              </div>
              <p className="text-sm text-slate-600 mb-3">{t(`arch.layers.${layer.id}.role`)}</p>
