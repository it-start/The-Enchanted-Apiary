      ...
      <div className="relative z-10 flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 space-y-8">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-3xl font-serif font-bold text-white mb-2 flex items-center gap-3">
                <Shield className="text-blue-400" /> {t('immune.status')}
              </h3>
              <div className="flex items-center gap-2 text-sm">
                 ...
                 <span className={isSimulation ? "text-amber-500" : "text-emerald-500"}>
                   {isSimulation ? t('immune.simulation') : t('immune.connected')}
                 </span>
          ...
          <div className="space-y-4">
            <h4 className="text-sm font-bold uppercase tracking-widest text-slate-500">{t('immune.mutations')}</h4>
            {loading ? (
              ...
              <div className="p-6 bg-emerald-900/20 border border-emerald-500/30 rounded-xl flex items-center gap-4">
                ...
                <div>
                  <div className="font-bold text-emerald-200">{t('immune.healthy')}</div>
                  <div className="text-sm text-emerald-400/70">{t('immune.healthyDesc')}</div>
                </div>
              </div>
            ) : (
              <div className="grid gap-3">
                {issues.map(issue => (
                  <div key={issue.id} className="p-4 bg-slate-800/50 border border-slate-700 rounded-xl flex items-center justify-between group hover:border-red-500/50 transition-colors">
                    ...
                    >
                      <GitPullRequest size={14} /> {t('immune.deploy')}
                    </button>
                  </div>
                ))}
              </div>
        ...
        <div className="lg:w-96 bg-black/50 rounded-xl border border-slate-800 p-4 font-mono text-xs flex flex-col h-[400px]">
          <div className="flex items-center gap-2 text-slate-500 mb-4 pb-2 border-b border-slate-800">
            <Terminal size={14} />
            <span>{t('immune.log')}</span>
          </div>
          <div className="flex-1 overflow-y-auto space-y-3 scrollbar-hide">
            {logs.length === 0 && (
              <div className="text-slate-600 italic">{t('immune.idle')}</div>
            )}
            {logs.map((log, i) => (
